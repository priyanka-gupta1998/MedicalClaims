 ...
 *//**
	 ...
	 *//*
		 ...
		 * List<ClaimDetails> claimDet; ClaimDetails claimDetails; PolicyDetails
		 * polictDetails; Optional<List<ClaimDetails>> claimDetailsOptional;
		 * List<PolicyDetails> polictDetailss;
		 * 
		 * @Before public void setup() { claimDetailss = new ArrayList<>();
		 ...
		 * claimDetails.setPolicyDetails(polictDetails); claimDetailss=new
		 * ArrayList<>(); claimDto=new ClaimDto(); claimDto.setAdmittedDate(toDate);
		 * claimDto.setClaimDate(toDate1); claimDto.setDischargeDate(dischargeDate);
		 * 
		 ...
		 * Mockito.when(claimDetailsRepository.findByClaimStatus(approvalStatus).
		 * thenReturn(claimDetailsOptional); ClaimDetailsResponseDto
		 * claimDetailsResponseDto=claimDetailsServiceImpl.getClaimDetails();
